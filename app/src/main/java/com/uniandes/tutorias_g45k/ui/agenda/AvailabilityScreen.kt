    ...
    Column(modifier=modifier, verticalArrangement = Arrangement.SpaceEvenly){
        if (!isTutor){
            Box(modifier=Modifier.fillMaxSize(), contentAlignment = Alignment.Center){
                NoContentOrConnectionWidget(size=100, message="No eres tutor activo. Vuelvete tutor y define tu horario de disponibilidad.", text_style = MaterialTheme.typography.headlineMedium)
            }
        }else{
